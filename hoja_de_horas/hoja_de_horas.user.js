...
// Enable all input forms

var all_disabled_inputs, all_hidden_disabled_inputs, this_input;

all_disabled_inputs = xpath("//input[@disabled]");
all_hidden_disabled_inputs = xpath("//input[@value='Disabled']");

for (var i = 0; i < all_disabled_inputs.snapshotLength; i++) {
  var this_input = all_disabled_inputs.snapshotItem(i);
  this_input.removeAttribute('disabled');
}

for (var i = 0; i < all_hidden_disabled_inputs.snapshotLength; i++) {
  var this_input = all_hidden_disabled_inputs.snapshotItem(i);
  this_input.value='';
}
